...
K         = 1;   % Number 1of users
eq_mode   = 'ZF';  % 'MF', 'ZF', 'MMSE', 'MFDC'
nSymb     = 40;  % Number of OFDM symbols to transmit at each iteration
nIter     = 1;  % Averaging the results over nIter channel realizations
N_wrap    = 1;
N_arr     = 1;[K+1 2.^(3:1:9)+1  1000]; % Run the sims for an array of N (number of antennas)
...
L        = 1; % Nact/K;   %round(M/6);
act_subc = [1:ceil(Nact/2) (ceil(-Nact/2):-1)+M]' + 1; % Active subcarriers   %
act_subc = setdiff(act_subc,[7 8 9 21 22 23 43 44 45 57 58 59]);
Nact = length(act_subc);
% % act_subc = (1:M);
